<template>
    <div class="d-block">
    <div class="voltar border border-1 border-light rounded-circle px-3 py-2 float-start ms-3 position-absolute" @click="voltar()">
...
<div class="lista box w-50" >
        ...
        <div class="row mt-4 box" v-if="showTextArea">
            <textarea v-model="novoComentario" placeholder="Digite seu comentário aqui..." class="form-control w-75" id="comentario" rows="3"></textarea>
            <button type="button" rows="3" class="btn btn-primary" @click="validarComentario()">Publicar</button>
        </div>
  </div>
  </div>
  </div>
    <div class=" d-flex justify-content-center">
        <div class="lista box mt-2 text-center"> {{  comments.length==0 ? 'Esta postagem ainda não possui comentários.': 'Comentários:' }} </div>
    </div>

    <div class="d-flex justify-content-center mt-2" v-for="comment in comments" :key="comment.id">
        <ComentarioVue :comment="comment" />
    </div>
  </div>
</template>

<script>
import ComentarioVue from './Comentario.vue';
...
export default {
    ...
    methods: {
        ...
        async fetchPostById() {
            const req = await fetch("https://gorest.co.in/public/v2/posts/" + this.id , {
                method: "GET",
                headers: {
        ...
        async fetchPostUser() {
            ...
            const req = await fetch(`https://gorest.co.in/public/v2/users/${this.id_user}`, {
                method: "GET",
                headers: {
                    'Authorization': 'Bearer ' + '791a4bdc85e9e4db3defbdd204c01ee1fbd39c5faf755fda9a3979649e5a6881',
                    'Content-Type': 'application/json'
        ...
        async fetchComments() {
            const req = await fetch(`https://gorest.co.in/public/v2/posts/${this.id}/comments`, {
                method: "GET",
                headers: {
                    'Authorization': 'Bearer ' + '791a4bdc85e9e4db3defbdd204c01ee1fbd39c5faf755fda9a3979649e5a6881',
                    'Content-Type': 'application/json'
